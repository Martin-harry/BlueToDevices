            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    ...
                    case BleMsg.MSG_CHA_NOTIFY_RESULT: {
                        ...
                        Log.e("通知回调----", "状态值：" + status + "------" + notifyCallback);
                        if (notifyCallback != null) {
                            if (status == BluetoothGatt.GATT_SUCCESS) {//判断状态值

                                notifyCallback.onNotifySuccess();//打开通知成功
                    ...
                    case BleMsg.MSG_CHA_NOTIFY_DATA_CHANGE: {//设备发送过来的数据
                        BleNotifyCallback notifyCallback = (BleNotifyCallback) msg.obj;
                        Bundle bundle = msg.getData();
                        byte[] value = bundle.getByteArray(BleMsg.KEY_NOTIFY_BUNDLE_VALUE);
                        String hexStr = HexUtil.encodeHexStr(value);
                        Log.e("通知回调----", "handleMessage: "+hexStr);
                        if (notifyCallback != null) {
